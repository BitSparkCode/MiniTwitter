import { api, Post, Comment, ApiError } from '../api/client';
...
export async function renderProfile(userId?: number): Promise<void> {
  ...
  let activity: { posts: Post[]; comments: Comment[] } = { posts: [], comments: [] };
  let profileUser: { id: number; username: string; role: string; createdAt: string } = {
    id: targetId,
  ...
  const renderComments = () => {
    ...
    activity!.comments.forEach((c) => {
      ...
      if (isOwnProfile) {
        const bodyText = card.querySelector<HTMLElement>('.comment-body-text')!;
        const editArea = card.querySelector<HTMLElement>('.comment-edit-inline')!;
        const ta = card.querySelector<HTMLTextAreaElement>('.comment-edit-ta')!;
        const editBtn = card.querySelector<HTMLElement>('.btn-edit-inline')!;
        const saveBtn = card.querySelector<HTMLElement>('.btn-save-inline')!;
        const cancelBtn = card.querySelector<HTMLElement>('.btn-cancel-inline')!;
        const deleteBtn = card.querySelector<HTMLElement>('.btn-delete-inline')!;
  ...
  page.querySelectorAll<HTMLButtonElement>('.tab-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.tab === 'posts') renderPosts();
      else renderComments();
    });
  });

  if (isOwnProfile) {
    const usernameInput = page.querySelector<HTMLInputElement>('#new-username')!;
    const updateBtn = page.querySelector<HTMLButtonElement>('#btn-update-username')!;
    const msgEl = page.querySelector<HTMLElement>('#profile-msg')!;
  ...
  if (isAdmin && isOtherUser) {
    const adminMsg = page.querySelector<HTMLElement>('#admin-msg')!;

    const showAdminMsg = (msg: string, type: 'error' | 'success') => {
    ...
    page.querySelector('#btn-set-role')?.addEventListener('click', async () => {
      const role = page.querySelector<HTMLSelectElement>('#role-select')!.value;
      try {
