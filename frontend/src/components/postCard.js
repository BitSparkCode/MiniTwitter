...
export function createPostCard(opts) {
    ...
    const attachHandlers = () => {
        ...
        card.querySelector('.btn-save-post')?.addEventListener('click', async () => {
            const ta = card.querySelector(`#post-edit-input-${post.id}`);
        ...
        card.querySelector(`#btn-comments-${post.id}`)?.addEventListener('click', () => {
            const section = card.querySelector(`#comments-section-${post.id}`);
            commentsOpen = !commentsOpen;
            if (commentsOpen) {
                section.style.display = 'block';
                renderCommentList(section, post.id, (delta) => {
                    commentCount += delta;
                    const btn = card.querySelector(`#btn-comments-${post.id}`);
                    if (btn)
                        btn.textContent = `💬 ${commentCount} Comment${commentCount !== 1 ? 's' : ''}`;
                });
            }
            else {
